...
class SQLAlchemyTypeStep(ParserStep):
    ...
    def parse(  # noqa: C901, PLR0912
        ...
        if isinstance(input_type, sqltypes.DateTime):
            is_tz_aware, time_zone = input_type.timezone, get_anyschema_metadata(metadata, "time_zone")
            if is_tz_aware and (time_zone is None):
                msg = (
                    "SQLAlchemy `DateTime(timezone=True)` does not specify a fixed timezone.\n\n"
                    "Hint: You can specify a timezone via `Column(..., info={\n"
                    "    '__anyschema_metadata__': {'time_zone': 'UTC'}}` "
                    "or `mapped_column(..., info={'__anyschema_metadata__': {'time_zone': 'UTC'}})`."
                )
