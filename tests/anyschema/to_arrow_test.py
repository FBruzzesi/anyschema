...
class User(BaseModel):
    id: int = PydanticField(json_schema_extra={"__anyschema_metadata__": {"nullable": False, "description": "User ID"}})
    username: str = PydanticField(json_schema_extra={"__anyschema_metadata__": {"nullable": True}})
    email: str | None
...
class Product(BaseModel):
    name: str | None = PydanticField(
        json_schema_extra={
            "__anyschema_metadata__": {"nullable": False, "description": "Product name"},
            "max_length": 100,
        }
    )
    price: float = PydanticField(
        json_schema_extra={"__anyschema_metadata__": {"nullable": True}, "currency": "USD", "min": 0}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (User, (None, None, None)),  # description is now in AnyField.description, not custom metadata
        (Product, ({b"max_length": b"100"}, {b"currency": b"USD", b"min": b"0"})),  # description extracted to AnyField
    ],
)
