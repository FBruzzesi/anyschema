...
def test_parse_string_literal() -> None:
    ...
    schema = model_to_nw_schema(UserModel, pipeline=pipeline)
...
def test_parse_mixed_literal_types() -> None:
    ...
    schema = model_to_nw_schema(ConfigModel, pipeline=pipeline)
...
def test_parse_literal_with_optional() -> None:
    class ProductModel(BaseModel):
        name: str
        category: Literal["electronics", "clothing", "food"] | None
        priority: Literal["high", "medium", "low"]

    schema = model_to_nw_schema(ProductModel, pipeline=pipeline)
...
def test_parse_nested_model_with_literal() -> None:
    ...
    schema = model_to_nw_schema(PersonModel, pipeline=pipeline)
